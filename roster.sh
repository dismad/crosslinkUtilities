...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      echo "  -z, --zats   use get_tfl_roster_zats (voting_power in zatoshis)"
...
if $USE_ZATS; then
  # Shape: [{pub_key, voting_power, txids}, ...]
  echo "$BODY" | jq -r '
    if .error then
      ...
      .result
      | sort_by(.voting_power) | reverse
      | .[]
      | "  " + .pub_key + "  →  " + (.voting_power|tostring) + " zats"
    end
  '
else
  # Shape: [[pubkey, stake_zec], ...]
  echo "$BODY" | jq -r '
    if .error then
      "RPC error: " + (.error.message // (.error|tostring))
    elif .result == null then
